...
public abstract class GeoModel<T extends GeoAnimatable> {
	 ...
	 */
	public class_2960 getModelResource(T animatable, @Nullable GeoRenderer<T> renderer) {
	 ...
	 */
	public class_2960 getTextureResource(T animatable, @Nullable GeoRenderer<T> renderer) {
	...
	public Animation getAnimation(T animatable, String name) {
		...
		for (class_2960 fallbackLocation : getAnimationResourceFallbacks(animatable)) {
	...
	public void handleAnimations(T animatable, long instanceId, AnimationState<T> animationState, float partialTick) {
		...
		if (currentTick == null)
			currentTick = animatable instanceof class_1297 entity ? (double)entity.field_6012 : RenderUtil.getCurrentTick();

		if (animatableManager.getFirstTickTime() == -1)
			animatableManager.startedAt(currentTick + partialTick);

		double currentFrameTime = animatable instanceof class_1297 || animatable instanceof GeoReplacedEntity ? currentTick + partialTick : currentTick - animatableManager.getFirstTickTime();
